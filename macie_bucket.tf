...
resource "aws_s3_bucket" "macie_bucket" {
  ...
  bucket   = var.use_bucket_namespace ? "${var.macie_bucket_name}-${data.aws_caller_identity.security.account_id}-${data.aws_region.security.region}-an" : var.macie_bucket_name
